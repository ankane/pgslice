module PgSlice
  class CLI
    ...
    option :read_only, type: :boolean, default: false, desc: "Log SQL statements instead of executing them"
    def synchronize(table_name)
      ...
      log "Mode: #{read_only ? 'READ-ONLY (logging only)' : 'WRITE (executing changes)'}"
      ...
      loop do
        ...
        if fix_queries.any?
          ...
          if read_only
            log_sql "-- Read-only mode: logging statements (not executing)"
